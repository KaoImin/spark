...
async fn main() {
    ...
    let kvdb_smt = Arc::new(SmtManager::new(&config.smt_db()));
    let kvdb_status = Arc::new(KVDB::new(&config.status_db()));

    let sync = Synchronization::new(
        Arc::clone(&ckb_rpc_client),
        Arc::clone(&rdb),
        Arc::clone(&kvdb_status),
        config.start_number,
    )
    .await;
    ...
    let _handle = run_server(rdb, kvdb_status, ckb_rpc_client, config.rpc_listen_address)
        .await
        .unwrap();
